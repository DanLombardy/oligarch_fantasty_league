  ...
  return {
    then: function(callback) {
      ...
      rollCall.find(function (err, data) {
        ...
        var position;
        try {
        position = element.votes[0].vote.positions.filter(function (position) { return position.member_id == senator_id})[0].vote_position || 'n/a';
        }
        catch(err) {
        position = 'Legislator not serving during this vote.'
        }

        //get the description of the vote
        var description = element.votes[0].vote.description || 'n/a';

        //date of vote
        var date = element.votes[0].vote.date || 'n/a';
